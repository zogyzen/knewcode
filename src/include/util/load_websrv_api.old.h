...
namespace KC
{
    ...
    public:
        // 初始化
        void Init(std::string fxPath, const int version)
        {
            // 加载api动态库
            std::string sDllPath = fxPath + "/kc_websrv_api." + std::to_string(version);
            boost::system::error_code ec;
            std::cout << "Load Webserve API - " << sDllPath << std::endl;
            m_lib.reset(new KLoadLibrary<>(sDllPath.c_str()));
            string sErrMsg = m_lib->LoadLib();
            if (!sErrMsg.empty())
            {
            ...
            // 判断版本
            typedef int(*FGetApiVersion)(void);
            FGetApiVersion fGetApiVersion = m_lib->GetLibFunc<FGetApiVersion>("GetApiVersion", sErrMsg);
            if (nullptr == fGetApiVersion || !sErrMsg.empty())
            {
                std::cout << sErrMsg << std::endl;
                throw std::runtime_error(sErrMsg);
            }
            auto dttm = boost::posix_time::microsec_clock::local_time();
            auto dt = dttm.date();
            auto tm = dttm.time_of_day();
            string s = (boost::format("%04d.%02d.%02d.%02d%02d") % (int)(dt.year()) % (int)(dt.month()) % (int)(dt.day()) % (int)(tm.hours()) % (int)(tm.minutes())).str();
            cout << s << endl;

            const int apiVersion = fGetApiVersion();
            if (apiVersion != version)
                throw std::runtime_error((boost::format("Webserve API Version Inconformity: %d <> %d") % apiVersion % version).str());
            // 设置回调函数
            typedef int(*FSetCBFuncPointer)(int, const char*, void*);
            FSetCBFuncPointer fSetCBFuncPointer = m_lib->GetLibFunc<FSetCBFuncPointer>("SetCBFuncPointer", sErrMsg);
            if (nullptr == fSetCBFuncPointer || !sErrMsg.empty())
            {
                std::cout << sErrMsg << std::endl;
                throw std::runtime_error(sErrMsg);
            }
            {
                auto fsetcb = fSetCBFuncPointer;
                /////////////////////////////////// 设置回调函数 ///////////////////////////////////
                // 获取服务器配置信息的回调函数
            ...
            FInit fInit = m_lib->GetLibFunc<FInit>("Init", sErrMsg);
            if (nullptr == fInit || !sErrMsg.empty())
            {
                std::cout << sErrMsg << std::endl;
                throw std::runtime_error(sErrMsg);
            }
            const int iErrCode = fInit();
        ...
        void Free(void)
        {
            try
            {
                if (m_lib.get() != nullptr && m_lib->IsLoad())
                {
                    typedef int(*FFree)(void);
                    FFree fInit = m_lib->GetLibFunc<FFree>("Free");
                    if (nullptr != fInit) fInit();
                    m_lib.reset();
                }
            }
            catch (...) {}
        ...
        int Request(intptr_t hRequest)
        {
            ...
            typedef int(*FRequest)(intptr_t);
            string sErrMsg;
            FRequest fRequest = m_lib->GetLibFunc<FRequest>("Request", sErrMsg);
            if (nullptr == fRequest || !sErrMsg.empty())
            {
                std::cout << sErrMsg << std::endl;
                throw std::runtime_error(sErrMsg);
            }
            return fRequest(hRequest);
        }
        ...
        int WriteLog(int lv, const char* msg, const char* pos, const char* type) const
        {
            try
            {
                if (m_lib.get() != nullptr && m_lib->IsLoad())
                {
                    ...
                    string sErrMsg;
                    FWriteLog fWriteLog = m_lib->GetLibFunc<FWriteLog>("WriteLog", sErrMsg);
                    if (nullptr != fWriteLog)
                        return fWriteLog(lv, msg, pos, type);
                }
            }
            catch (...) {}
            return -1;
    ...
    protected:
        // 写临时日志
        static void WriteTmpLog(string type, string pos, string msg)
        {
            if (!TOwn::m_self.expired())
            {
                if (TOwn::m_self.lock()->m_load.WriteLog(4, msg.c_str(), pos.c_str(), type.c_str()) != 0)
                    CTempLog::WriteInDir(TOwn::m_self.lock()->m_FxPath, type, pos, msg);
            }
    ...
    protected:
        ...
        // 设置通过错误码判断是否掉线的回调函数指针类型。参数依次为：3；"FSetErrCodeIsDisconn"；错误码。返回：true已掉线；false未掉线。
        static bool IsDisconnByErrCodeCB(int argCount, const char* flag, int errCode)
        {
            try
            {
                if (CheckArgFlag(3, "FSetErrCodeIsDisconn", argCount, flag) && !TOwn::m_self.expired())
        ...
        static const char* GetStatusByErrCodeCB(int argCount, const char* flag, int errCode)
        {
            try
            {
                if (CheckArgFlag(3, "FSetGetStatusByErrCode", argCount, flag) && !TOwn::m_self.expired())
