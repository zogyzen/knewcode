...
namespace KC
{
    ...
    public:
#ifdef WIN32    // Windows环境
        static std::string _GetLoadDllError(void)
        {
            return "";
        }
#else           // linux环境
        static std::string _GetLoadDllError(void)
        {
            const char* pStr = dlerror();
            return nullptr != pStr ? pStr : "";
        }
#endif
    };
    ...
    public:
        ...
        string LoadLib(void)
        {
            ...
            if (0 == m_lib_handle)
            {
                m_lib_handle = ::LoadLibraryA(m_strLibFileName.c_str());
                if (0 == m_lib_handle)
        ...
        // 返回库文件名
        const char* GetLibFileName(void) const { return m_strLibFileName.c_str(); }
        // 最近访问时间
    ...
    private:
        ...
        #endif
		// 库扩展名
        const char* m_LibExtName = c_so_ext_name;
        // 库文件全名
