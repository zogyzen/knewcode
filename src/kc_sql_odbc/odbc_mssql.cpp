...
void TParmODBCmssql::SetString(string strVal)
{
    //if (CSqlFunc::EParmType::eptOutParm != ePTp && CSqlFunc::EParmType::eptInOutParm != ePTp)
    if (CSqlFunc::EParmType::eptOutParm != ePTp)
        m_own.m_qry.bindValue((":" + sName).c_str(), strVal.c_str(), QSql::In);
}
void TParmODBCmssql::SetClob(string strVal)
{
...
string TParmODBCmssql::GetString(string sDef)
{
    ...
    */
    ...
    // 输出参数（直接取）
        ...
        // return sVal;
        return BoostAnyToStdString(anyVal);
    return sDef;
}
...
string TParmODBCmssql::GetDate(string fmt)
{
    ...
    */
    ...
    if (m_own.CtrlD().ActionData().IsGlobalVal(sName.c_str()))
    {
        auto it = m_own.mapParms.find(sName);
        if (m_own.mapParms.end() != it && !it->second->bIsNull)
            // val = it->second->sVal.c_str();
            val = QString(BoostAnyToStdString(it->second->anyVal).c_str());
    }
    // 输出参数（直接取）
    else if (!bIsNull)
        // val = sVal.c_str();
        val = QString(BoostAnyToStdString(anyVal).c_str());
...
int TParmODBCmssql::GetInt(int iDef)
{
    ...
    */
    ...
    if (m_own.CtrlD().ActionData().IsGlobalVal(sName.c_str()))
    {
        auto it = m_own.mapParms.find(sName);
        if (m_own.mapParms.end() != it && !it->second->bIsNull)
            // return atoi(it->second->sVal.c_str());
            return static_cast<int>(CastBoostAnyToInt(it->second->anyVal));
    }
    // 输出参数（直接取）
    else if (!bIsNull)
        // return atoi(sVal.c_str());
        return static_cast<int>(CastBoostAnyToInt(anyVal));
    return iDef;
}
...
double TParmODBCmssql::GetNumber(double fDef)
{
    ...
    */
    ...
    if (m_own.CtrlD().ActionData().IsGlobalVal(sName.c_str()))
    {
        auto it = m_own.mapParms.find(sName);
        if (m_own.mapParms.end() != it && !it->second->bIsNull)
            // return atof(it->second->sVal.c_str());
            return CastBoostAnyToFloat(it->second->anyVal);
    }
    // 输出参数（直接取）
    else if (!bIsNull)
        // return atof(sVal.c_str());
        return CastBoostAnyToFloat(anyVal);
    return fDef;
}
...
CSqlFunc::TRecordSetPtr TParmODBCmssql::GetDbSet(void)
{
    try
    {
        ...
        if (!qry.exec(("select * from " + sTmpName).c_str()))
            throw qry.lastError();
        return CSqlFunc::TRecordSetPtr(new TRecordSetODBCmssql(m_own.m_own, qry, m_own.m_objCtrlD));
    }
    catch (QSqlError& ex)
    {
...
TParmGrpODBCmssql::TParmGrpODBCmssql(CCtrlODBC& own, TOdbcConnObj& db, QSqlQuery& q, ICtrlApiData& ctrlD, string sAct)
    : TParmGrpODBCmssqlBase(true, false, false), m_own(own), m_connObj(db), m_qry(q), m_objCtrlD(ctrlD), m_act(sAct)
{
}
...
void TParmGrpODBCmssql::ParseSqlParms(std::string, std::string)
{
    ...
    unsigned i = 1;
    QMap<QString, QVariant> m = m_qry.boundValues();
    for (auto &p : m.keys())
    {
        qDebug() << p << "\n";
...
TRecordSetODBCmssql::TRecordSetODBCmssql(CCtrlODBC& own, QSqlQuery q, ICtrlApiData& ctrlD)
    : m_own(own), m_objCtrlD(ctrlD), m_qry(q)
{
}
TRecordSetODBCmssql::~TRecordSetODBCmssql(void)
{
}

// 获取字段信息
void TRecordSetODBCmssql::GetColumnListMetaData(CSqlFunc::TFields &vecFeilds)
{
...
// 下一条
bool TRecordSetODBCmssql::Next(void)
{
    return m_qry.next();
}
// 添加值
void TRecordSetODBCmssql::AddVal(IKCJson& jsonRow, CSqlFunc::TField& fld)
{
    if (!m_qry.isNull(fld.m_name.c_str()))
        switch (fld.m_type)
        {
        case QMetaType::Double:
            jsonRow.AddVal(fld.m_name.c_str(), m_qry.value(fld.m_name.c_str()).toDouble());
            break;
        case QMetaType::Int:
        case QMetaType::UInt:
        case QMetaType::LongLong:
        case QMetaType::ULongLong:
            jsonRow.AddVal(fld.m_name.c_str(), m_qry.value(fld.m_name.c_str()).toInt());
            break;
        case QMetaType::QDate:
            jsonRow.AddStr(fld.m_name.c_str(), m_qry.value(fld.m_name.c_str()).toDate().toString("yyyy-MM-dd").toStdString().c_str());
            break;
        case QMetaType::QDateTime:
            jsonRow.AddStr(fld.m_name.c_str(), m_qry.value(fld.m_name.c_str()).toDateTime().toString("yyyy-MM-dd hh:mm:ss").toStdString().c_str());
            break;
        default:
            jsonRow.AddStr(fld.m_name.c_str(), m_qry.value(fld.m_name.c_str()).toString().toStdString().c_str());
            break;
        }
    else jsonRow.AddNull(fld.m_name.c_str());
}
// 得到值
CSqlFunc::TKcValVariant TRecordSetODBCmssql::GetVal(CSqlFunc::TField& fld) const
{
    ...
    if (!m_qry.isNull(fld.m_name.c_str()))
        ...
        {
        case QMetaType::Double:
            result = m_qry.value(fld.m_name.c_str()).toDouble();
            break;
        case QMetaType::Int:
        case QMetaType::UInt:
        case QMetaType::LongLong:
        case QMetaType::ULongLong:
            result = m_qry.value(fld.m_name.c_str()).toInt();
            break;
        case QMetaType::QDate:
            result = m_qry.value(fld.m_name.c_str()).toDate().toString("yyyy-MM-dd").toStdString();
            break;
        case QMetaType::QDateTime:
            result = m_qry.value(fld.m_name.c_str()).toDateTime().toString("yyyy-MM-dd hh:mm:ss").toStdString();
            break;
        default:
            result = m_qry.value(fld.m_name.c_str()).toString().toStdString();
            break;
        }
    return result;
}
...
void TDBCommandODBCmssql::PrepareSQL(void)
{
    ...
    if (c_RESTful_batch != m_method && c_RESTful_create != m_method)
    {
        auto fGetParmType = [&](CSqlFunc::TParmPtr p)
        {
            ...
            {
                if (p->anyVal.type() == typeid(int) || p->anyVal.type() == typeid(long long))
                    eDTp = CSqlFunc::EDataType::edtInt;
                else if (p->anyVal.type() == typeid(double))
                    eDTp = CSqlFunc::EDataType::edtNumber;
                else eDTp = CSqlFunc::EDataType::edtString;
            }
            // 返回类型
            string sType = "varchar(max)";
            if (CSqlFunc::EDataType::edtInt == eDTp) sType = "int";
            else if (CSqlFunc::EDataType::edtNumber == eDTp) sType = "numeric(18, 4)";
            else if (CSqlFunc::EDataType::edtDatetime == eDTp) sType = "datetime";
            return sType;
        };
        ...
        // 删除返回参数的临时表
        m_connObj.m_odbc.exec(("if not object_id('" + m_parmTmpTab + "') is null drop table " + m_parmTmpTab).c_str());
    }
    // 重新预执行sql语句
...
{
    m_qry.finish();
    CAutoRelease _auto([&](){ m_connObj.m_odbc.exec(("if not object_id('" + m_parmTmpTab + "') is null drop table " + m_parmTmpTab).c_str()); });
...
void TDBCommandODBCmssql::MakeParmVals(void)
{
    MakeParmVals([&](string sParmName, QVariant val){
        ...
        if (m_parms.mapParms.end() != itParm)
        {
            ...
            if (itParm->second->bIsNull)
                // itParm->second->sVal.clear();
                itParm->second->anyVal.clear();
            else
            {
                // itParm->second->sVal = val.toString().toStdString();
                // 按类型赋值
