...
void CCtrlODBC::Perform(ICtrlApiData& objCtrlD, IKCController::IAttachParm&)
{
    ...
    string sActObjName = m_name + "@" + CUtilFunc::PCharSafeToStr(m_own.getAlias(), "kc_sql_odbc") + "-" + m_dbType;
    ...
    catch (TKCSqlException& ex)
    {
        ...
        string sErr = "[" + std::to_string(ex.error_id()) + "]" + sEx + fExceptInfo();
        m_own.WriteLogFatal(sErr.c_str(), __CURR_CODE_PLACE_C__, typeid(ex).name());
    ...
    catch (...)
    {
        ...
        objCtrlD.SetJsonRespond(ecd_ErrCode_KCSqlODBC + 13, ("[Controllers." + sAct + "] " + m_own.getHint("Unknown_exception") + " (" + sActObjName + ")").c_str());
    }
}
