...
////////////////////////////////////////////////////////////////////////////////
// TVarDouble类
KC::one_level::TVarDouble::TVarDouble(IIDNameItemWork& wk, IKCActionData& act, string name)
    : TVarItemBase(wk, act, EKcDtDouble, name)
{
}

// 变量赋值
void KC::one_level::TVarDouble::SetVarValue(boost::any val)
{
    if (val.empty())
        m_value = 0;
    else if (val.type() == typeid(double))
        m_value = boost::any_cast<double>(val);
    ...
    else if (val.type() == typeid(string))
    {
        ...
        try
        {
            m_value = lexical_cast<double>(strVal);
        }
        catch(...)
        {
...
// 获取变量的值
boost::any KC::one_level::TVarDouble::GetVarValue(void)
{
    return m_value;
}
